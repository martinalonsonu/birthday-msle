...
imageFiles.forEach(async (file) => {
  ...
  try {
    ...
    console.log(`✓ ${file} → ${path.basename(outputPath)}: ${(originalSize / 1024).toFixed(1)}KB → ${(newSize / 1024).toFixed(1)}KB (${reduction}% smaller)`);
  } catch (error) {
    console.error(`✗ Error processing ${file}:`, error.message);
  }
});

console.log('WebP conversion complete! You can now use these files as alternatives to JPEG.');
